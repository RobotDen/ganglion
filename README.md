...
Keys: `↑↓`/`j k` select a peer · `⏎` inspect it · `p` pause the feed (for a clean recording) · `/` filter · `a` audit-only fullscreen · `?` help · `q`/Esc quit. The feed is a genuine server-push substream ([ADR-024](docs/adr/ADR-024-event-push-stream.md)), so events land the instant the robot emits them; the `♥ live` pulse (driven by 15 s heartbeats) shows it is fresh. Honors `NO_COLOR` (monochrome/ASCII) and resizes gracefully. Full reference: [docs/CLI_REFERENCE.md](docs/CLI_REFERENCE.md).
