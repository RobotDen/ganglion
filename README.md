...
Keys: `↑↓`/`j k` select a peer · `⏎` inspect it · `p` pause the feed (for a clean recording) · `/` filter · `a` audit-only fullscreen · `?` help · `q`/Esc quit. The feed defaults to a genuine server-push substream ([ADR-024](docs/adr/ADR-024-event-push-stream.md)), so events land the instant the robot emits them, with automatic fallback to the request-response poll when push is unavailable (`--events-transport auto|push|poll`); a subtle `feed push`/`feed poll` indicator shows which is live. The `♥ live` pulse (driven by 15 s heartbeats) shows it is fresh. Honors `NO_COLOR` (monochrome/ASCII) and resizes gracefully. Full reference: [docs/CLI_REFERENCE.md](docs/CLI_REFERENCE.md).
